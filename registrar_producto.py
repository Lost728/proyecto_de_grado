...
def obtener_pixmap_fondo():
    """Obtiene el fondo desde la base de datos (imagen 'mar')."""
    try:
...
class InsertarProductoWindow(QMainWindow):
    def __init__(self, id_empleado=None):
        ...
        # Fondo gradiente púrpura-azul (sin imagen)
        self.setStyleSheet("""
            QMainWindow, QWidget {
            ...
            QPushButton#limpiar {
                background: qlineargradient(x1:0,y1:0,x2:1,y2:0, stop:0 #ffa69e, stop:1 #ffb6e6);
                color:#22223b;
            }
            QPushButton#volver {
    ...
    def guardar_producto(self):
        ...
        if not isinstance(datos["fecha_vencimiento"], datetime) and datos["fecha_vencimiento"] < hoy:
            QMessageBox.warning(self, "Advertencia", "La fecha de vencimiento no puede ser anterior a hoy.")
            return
        try:
            precio = float(datos["precio"])
            cajas = int(datos["cajas"]) if datos["cajas"] else 0
            paquetes_por_caja = int(datos["paquetes_por_caja"]) if datos["paquetes_por_caja"] else 0
            unidades_por_paquete = int(datos["unidades_por_paquete"]) if datos["unidades_por_paquete"] else 0
            unidades = int(datos["unidades"]) if datos["unidades"] else 0
        except ValueError:
    ...
    def tomar_foto(self):
        try:
            ...
            QMessageBox.warning(self, "Error", f"Error al tomar foto: {e}")
...
def abrir_aplicacion(nombre_py, argumentos=None):
    ...
    for base_path in base_paths:
        ...
        if os.path.exists(exe_path):
            ...
            subprocess.Popen(cmd)
            return
    # Si hay una ventana activa, mostrar el mensaje ahí
    app = QApplication.instance()
    parent = app.activeWindow() if app else None
    QMessageBox.warning(parent, "⚠️ Archivo no encontrado",
